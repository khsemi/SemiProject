$(function(){
	...
	if(startPage > 1){
		$("#pasing").append("<li class=''><a href='controller.do?category="+category+"&page=1&searchType="+searchType+"&keyword="+keyword+"&sortType="+sortType+"' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></a></li>");
	}else{
		$("#pasing").append("<li class='disabled'><a href='#' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></a></li>");
	}
	
	if(page > 1){
		$("#pasing").append("<li class=''><a href='controller.do?category="+category+"&page="+(page-1)+"&searchType="+searchType+"&keyword="+keyword+"&sortType="+sortType+"' aria-label='Previous'><span aria-hidden='true'>&lang;</span></a></li>");
	}else{
	...
	for(var iCount = startPage; iCount <= endPage; iCount++) {
		if (iCount == page) {
	       $("#pasing").append("<li class='active'><a href='#'>"+iCount+"<span class='sr-only'></span></a></li>");
	    	...
	    	$("#pasing").append("<li class=''><a href='controller.do?category="+category+"&page="+iCount+"&searchType="+searchType+"&keyword="+keyword+"&sortType="+sortType+"'>" + iCount + "<span class='sr-only'></span></a></li>");
	    }
	}
	
	if(page < totalPage){
		$("#pasing").append("<li class=''><a href='controller.do?category="+category+"&page="+(Number(page)+1)+"&searchType="+searchType+"&keyword="+keyword+"&sortType="+sortType+"' aria-label='Next'><span aria-hidden='true'>&rang;</span></a></li>");
	}else{
		$("#pasing").append("<li class='disabled'><a href='#' aria-label='Next'><span aria-hidden='true'>&rang;</span></a></li>");
	}
	
	if(endPage < totalPage){
		$("#pasing").append("<li class=''><a href='controller.do?category="+category+"&page="+totalPage+"&searchType="+searchType+"&keyword="+keyword+"&sortType="+sortType+"' aria-label='Next'><span aria-hidden='true'>&raquo;</span></a></li>");
	}else{
